...
def get_user(token):
    ...
    user = requests.get(
        ...
        params={
          '$select': 'displayName,mail,mailboxSettings,userPrincipalName,photo,country'
        })
    print(user)
    # Return the JSON result
    return user.json()

def get_photo(token, user):
    user_photo = requests.get(
    ...
    if user_photo.status_code == 200:
        # Save the photo to a file or process it as needed
        global pfp_path
        global pfp_name
        pfp_path = f'tutorial/static/tmp/{user["displayName"]}-{user["id"]}.jpg'
        pfp_name = f'{user["displayName"]}-{user["id"]}.jpg'
        with open(pfp_path, 'wb') as f:
            f.write(user_photo.content)
        print("Profile photo saved successfully.")
    else:
        print("Failed to retrieve profile photo. Status code:", user_photo.status_code)

    # Return the JSON result
    return user_photo.headers, pfp_name
...
def create_event(token, subject, start, end, attendees=None, body=None, timezone='UTC'):
    ...
    requests.post(f'{GRAPH_URL}/me/events',
        headers=headers,
